...
[MCP の仕様バージョン](https://modelcontextprotocol.io/docs/learn/versioning)は、`YYYY-MM-DD` 形式です。この日付は、互換性を壊す変更が最後に行われた日を示します。
...
次の図は、セッション ID でセッションを管理する構成で、ロードバランサーがリクエストを 2 台のサーバーへ振り分ける例です。ロードバランサーとは、届いたリクエストを複数のサーバーへ分配する仕組みです。
...
各リクエストに必要なプロトコル情報が入るため、同じクライアントからのリクエストを特定のサーバーへ固定する必要がなくなります。一般的なロードバランサーで複数のサーバーへ振り分けられ、MCP のプロトコル上のセッション情報を共有するストレージも不要です。ただし、検索の続きなど、アプリ自身の処理状況を保存するストレージは必要に応じて別途用意します。

通常のリクエストでは、サーバーからクライアントへ別のリクエストを送るために通信を開いたままにする前提もなくなりました。ただし、変更通知を継続して受け取る `subscriptions/listen` は、後述するように応答を開いたままにします。

リクエストを独立して処理できる設計は、リクエストを受けたときだけ処理を実行する環境と相性がよいと考えられます。たとえば、Cloudflare Workers のようなリクエスト単位の実行環境や、トラフィックがなければ[デフォルトでインスタンス数が 0 になる Google Cloud Run](https://docs.cloud.google.com/run/docs/overview/what-is-cloud-run#scale-to-zero)で構成しやすくなります。

Cloudflare も、新仕様への[対応発表](https://blog.cloudflare.com/mcp-v2/)で、MCP プロトコルを扱うためだけの Durable Objects は不要になり、MCP サーバーを 1 つの Worker で動かせると説明しています。Durable Objects とは、Cloudflare が提供する、状態の保持に使う仕組みです。アプリ自身が状態を必要とする場合には、引き続き Durable Objects が適しているとも説明しています。
...
この 2 つの廃止は、[SEP-2575](https://modelcontextprotocol.io/seps/2575-stateless-mcp) と [SEP-2567](https://modelcontextprotocol.io/seps/2567-sessionless-mcp) という提案に基づきます。SEP（Specification Enhancement Proposal）とは、MCP の仕様変更を提案する文書です。

旧方式では、`initialize` を送って応答を待ち、`notifications/initialized` を送ってから、ようやく本来のリクエストを送っていました。新方式では、最初から本来のリクエストを送れます。
...
| `io.modelcontextprotocol/clientInfo` | 推奨（設定により省略可） | クライアント名とバージョン |
...
ツールの実行中に、ユーザーの確認や不足している情報が必要になることがあります。たとえば、ファイルを削除するツールが、実行前に「本当に削除しますか？」と確認する場合です。MCP には、この追加情報をサーバーからクライアントへ求める仕組みがあります。
...
旧仕様では、元のリクエストを処理している途中で、サーバーがクライアントへ別のリクエストを送っていました。Streamable HTTP では、そのリクエストを届けるために SSE の通信を開いたままにする必要がありました。サーバー側では、クライアントから回答が届くまで、処理の途中経過を保持する必要もありました。
...
最初のリクエストと再送したリクエストは、それぞれ独立しています。最初に応答したサーバーのメモリだけに途中経過を残す設計では、別のサーバーが再送を受け取ったときに処理を再開できません。サーバーは、再開に必要な情報を `requestState` 自体に含めるか、`requestState` を手がかりに共有ストレージから取得できるようにします。こうしておけば、ロードバランサー配下では、最初に応答したサーバーとは別のサーバーでも再送を処理できます。
...
クライアントは、リクエストごとの `_meta` にある `io.modelcontextprotocol/clientCapabilities` の `extensions` へ、`io.modelcontextprotocol/tasks` を含めます。サーバーも `server/discover` の `capabilities.extensions` で対応を示します。

サーバーが `"task"` を返せるのは、クライアントとサーバーの両方が Tasks 拡張に対応している場合だけです。

以前の仕様には `resultType` がありません。クライアントが以前の仕様に沿って通信するサーバーから `resultType` のない結果を受け取った場合は、処理が完了したことを示す `"complete"` として扱う必要があります。`2026-07-28` に沿った結果では `resultType` が必須です。
...
Streamable HTTP では、ツール実行の進捗を知らせる通知は、元のリクエストに対応する応答ストリームで届きます。ログ通知を要求した場合、そのログも同じ応答ストリームで届きます。
...
- 認可サーバー（ログインを処理し、アクセスに必要な情報を発行するサーバー）へクライアントを自動登録する **Dynamic Client Registration（RFC 7591）** — 新しい実装では、クライアント情報を HTTPS 上の文書で公開する [Client ID Metadata Documents](https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization/client-registration#client-id-metadata-documents) を使う。対応していない認可サーバーとの互換性を保つ場合は、Dynamic Client Registration を引き続き使える

今回、機能を「有効（Active）」「非推奨（Deprecated）」「削除済み（Removed）」の 3 状態で管理するルールも決まりました。新たに非推奨にする機能は、原則として削除の候補になるまで 12 か月以上維持されます。

セキュリティ勧告が公開された脆弱性や実際の悪用があり、既存機能のままでは対処できない場合は、削除候補になるまでの期間を最短 90 日へ短縮できます。また、ルールができる前から非推奨だった HTTP+SSE には、別の移行規定があります。機能ごとの最短時期は、公式の[非推奨機能の一覧](https://modelcontextprotocol.io/specification/2026-07-28/deprecated)で確認します。
...
TypeScript SDK v2 には Node.js 20 以上が必要です。v2 では、1 つの `@modelcontextprotocol/sdk` パッケージが、サーバー用の `@modelcontextprotocol/server` とクライアント用の `@modelcontextprotocol/client` などへ分割されました。v1 からの機械的な書き換えには、パッケージのルートで `npx @modelcontextprotocol/codemod@latest v1-to-v2 .` を実行します。末尾の `.` は、現在のパッケージ全体を変換の対象にする指定です。

移行ツールだけでは書き換えられない箇所もあります。実行後は[公式の移行ガイド](https://ts.sdk.modelcontextprotocol.io/v2/migration/upgrade-to-v2)に沿って `@mcp-codemod-error` を検索し、型チェックとテストを行います。

なお、TypeScript SDK を v2 へ更新しただけでは、新方式では通信しません。[`2026-07-28` で通信するには、明示的な設定が必要です](https://ts.sdk.modelcontextprotocol.io/v2/migration/support-2026-07-28)。クライアントは `versionNegotiation` を設定します。HTTP サーバーは `createMcpHandler`、stdio サーバーは `serveStdio` を入口として使います。

### 5-2. セッションに保存していた情報を洗い出す

セッションに保存していた情報を、次の 2 種類に分けて確認します。

- **プロトコルの情報**：バージョン、クライアントの機能、クライアント名など。新仕様では、リクエストごとの `_meta` から取得する。クライアント名とバージョンの追加は推奨だが、設定により省略できる
...
`tools/list` は、毎回同じ順序で返すことが推奨されています。たとえば、同じツールでも「A、B」と「B、A」では別の文字列になります。順序を固定すると、クライアントが前回の一覧を再利用しやすくなり、LLM へ渡す内容も同じ形を保ちやすくなります。
...
本文と同じ値をヘッダーにも入れるのは、ロードバランサーやゲートウェイのような中間装置のためです。中間装置は、JSON 本文を解析しなくても、ヘッダーだけで振り分けや制限を判断できます。
...
事前登録または Dynamic Client Registration で発行されたクライアント ID などは、発行した認可サーバーごとに保存する必要があります。別の認可サーバーでは使い回してはいけません。
...
- [Versioning（ドキュメント）](https://modelcontextprotocol.io/docs/learn/versioning) — バージョン形式と日付の意味
...
- [SEP-2596: Specification Feature Lifecycle and Deprecation Policy](https://modelcontextprotocol.io/seps/2596-spec-feature-lifecycle-and-deprecation) — 非推奨から削除までのルール
...
- [SEP-2575: Make MCP Stateless](https://modelcontextprotocol.io/seps/2575-stateless-mcp) — 初期化を廃止し、リクエスト単位で処理する設計へ変更した経緯
- [SEP-2567: Sessionless MCP via Explicit State Handles](https://modelcontextprotocol.io/seps/2567-sessionless-mcp) — プロトコル上のセッションを廃止した経緯
- [The next generation of MCP](https://blog.cloudflare.com/mcp-v2/) — Cloudflare による新仕様への対応発表
- [What is Cloud Run](https://docs.cloud.google.com/run/docs/overview/what-is-cloud-run#scale-to-zero) — Google Cloud Run のスケール設定
