...
旧仕様では、ハンドシェイクで確認した内容を、その後のリクエストでも使い続けていました。HTTP では、サーバーが `Mcp-Session-Id` を発行し、複数のリクエストを同じセッションとして扱うこともできました。

この方式でサーバーを複数台に増やすと、ハンドシェイクの情報を持たないサーバーへ次のリクエストが届き、正しく処理できないことがあります。これを防ぐには、情報を共有する保存場所を用意するか、同じクライアントからのリクエストを同じサーバーへ送り続ける必要があります。
