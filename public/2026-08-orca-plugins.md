...
Orca のマーケットプレイスは、VS Code の拡張機能ストアとは仕組みが異なります。プラグイン本体の保管場所ではなく、各プラグインの取得先をまとめた一覧です。

> **注意:** プラグイン機能は experimental（試験的な機能）です。以下の内容は、2026年8月時点の macOS 版で確認した結果です。バージョンによってファイル形式や画面表示が変わる可能性があります。
...
日本語にすると、「プラグインごとに利用者の同意を得て有効化し、マーケットプレイスからコンテンツパック、パネル、ワーカーをインストールできる」という内容です。

先に結論をまとめます。

- マーケットプレイスの実体は Git リポジトリで、中身は各プラグインの取得先をまとめた一覧でした
- プラグインのマニフェストは VS Code 拡張機能のものとよく似ていました
- 公式マーケットプレイスの一覧には8件あるのに、Orca の画面に表示されるのは3件だけでした

この記事を読むうえで、プラグイン開発の知識は必要ありません。設定項目の役割から順に見ていきます。

## 全体像：マーケットプレイスはプラグインの案内役
...
一覧では、1つのプラグインに複数のカテゴリを付けられます。たとえばテーマには、種類を表す `themes` と公式カテゴリの `official` が付いています。

```json
"categories": ["themes", "official"]
```

`official` は8件すべてに付いています。
...
検索や絞り込みは使っておらず、リポジトリにも8件登録されています。Orca v1.4.173 が未対応の種類を除外している可能性はありますが、公式の説明は見つかっていません。確認できたのは、「一覧は8件、画面は3件」という事実だけです。
...
`Orca Navigation Shortcuts` は利用者が追加したものではありません。Orca は、最初から入っている機能もプラグインとして管理しています。
...
公式マーケットプレイスの実体は、この Git リポジトリです。Orca はその内容を取得し、PC 内に保存します。
...
`url` はプラグイン専用の Git リポジトリを指し、`ref: v1.0.0` は取得するバージョンを `v1.0.0` タグに指定しています。
...
- **審査済みかどうかは JSON だけでは判断できない**：一覧には審査状態を表す項目がありません。`official` もカテゴリに書かれた文字列です。公式マーケットプレイスが運用上どのように審査しているかは確認できませんでした
- **取得するバージョンを固定できる**：一覧が `v1.0.0` を指している間は、作者が新しいバージョンを公開しても取得対象は変わりません

バージョンを固定すると、新版が公開されても利用者の環境はすぐには変わりません。ただし、Git のタグ自体は付け替えられるため、コミットハッシュが変更を追跡する手がかりになります。
...
この2つがあれば、「どの一覧を見て、どのプラグインを入れたか」を後からたどれます。タグが付け替えられても、記録済みのコミットハッシュから当時の状態を識別できます。
...
ここでは JSON 全体を理解する必要はありません。主な項目と、VS Code の拡張マニフェストとの対応は次のとおりです。
...
確認した3件を比べると、いずれも `capabilities` は `[]` でした。これは、要求する権限がないことを表します。
...
残りの5件は確認していないため、公式の8件すべてが空とは断定できません。
...
この画面では、VM レシピが1件追加されることを確認できます。`capabilities` は空なので、権限に関する項目は表示されません。
...
インストール済みプラグインの情報は `plugins.lock.json` に保存されます。そこには `contentHash` と `capabilityHash` という2つのハッシュがありました。まず、同梱の `Orca Navigation Shortcuts` の記録を例に見ます。
...
この構造なら、更新前後のファイルを別々のディレクトリに保存できます。使用するディレクトリは、`current` の値で切り替えられます。

### `capabilityHash` は同意の記録と対応する

`capabilityHash` と同じ値は、インストール元の情報を保存する `.install-provenance/` にもあり、こちらでは `consentFingerprint`（同意内容の指紋）という名前が付いています。

名前だけを見ると、`capabilities` の内容から計算した値のように思えます。しかし、2件のプラグインを比べると、それだけでは説明できません。
...
どちらも `capabilities` は空ですが、`capabilityHash` は異なります。権限の一覧だけから計算しているなら、同じ値になるはずです。

インストール時のダイアログには、`contributes` に書かれた追加内容も表示されます。そのため、`capabilityHash` には権限だけでなく、プラグインが追加する内容も反映されている可能性があります。
...
`pluginConsents` にあるのは、自分で追加した `orca-multipass-recipes` だけで、同梱の `orca-navigation-shortcuts` はありません。利用者がインストールに同意したプラグインを記録する項目と考えられます。

Orca がこの値をいつ、どのように照合するかは確認できませんでした。ただし、現在の値と記録済みの値を比べれば、同意した後に確認対象が変わったかどうかを検出できます。

今回確認したプラグインは権限を要求していませんが、将来の権限追加や内容変更を区別できる形で同意の記録が残っていました。
...
今回確認できたのは、テーマや設定ファイルなど、Orca の既存機能が読み込むデータを追加するプラグインが中心です。changelog にあるパネルやワーカーの実例は、公式マーケットプレイスにはまだありませんでした。

一方で、マニフェストには権限を宣言する欄があり、同意した内容を識別するハッシュも残ります。今回確認したデータ追加型のプラグインだけでなく、より多くの権限を使うプラグインも扱える設計であることがうかがえます。
