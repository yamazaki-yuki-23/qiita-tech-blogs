...
デモアプリで試したとき、ボタン要素をクリックするとエージェントへの入力欄に `<button id="btn">` のHTMLとセレクタ情報が自動で入った。「このボタンの背景色を変えたい」と伝えるだけで、セレクタを手書きする必要がない。
...
言葉で伝えにくい位置ズレやマージン崩れは、スクリーンショットと要素のHTMLを両方渡すことでエージェントが構造を把握しやすくなる。ただしこの機能を使うには内蔵Chromiumでdevサーバーを開いておく必要があるため、静的HTMLを直接開いただけでは使えない点は注意が必要だ。

## GitHubとLinearの統合

GitHubとLinearにネイティブ対応しており、PRやIssue、プロジェクトボードをOrcaの中で確認できる機能がある。今回の検証では個人のデモプロジェクトで試したため、この統合は実際には試していない。

ただ、Claude Codeを実務で使っていると「issueを確認してブランチを切る → エージェントに実装を依頼する」という繰り返しが生まれる。ブラウザとターミナルを行き来するコストは地味に積み上がるので、チーム運用でOrcaを使い込む段階では恩恵が出る機能だと見ている。
...
diffビューはGitHubのPRレビュー画面に近いUIで、行単位の緑・赤ハイライトになっている。「この変数名を直してほしい」「この処理には副作用がある」といったコメントをdiffの該当行に紐付けて残せる。レビューが終わったらコメントをまとめてエージェントに送信して再実装させる、というサイクルが一画面で回る。
