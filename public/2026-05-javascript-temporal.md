...
- 2026-05-14 リリースの Node.js v26 で `Temporal` がフラグなしのデフォルト有効になった。Firefox 139（2025-05）・Chrome 144 / Edge 144（2026-04）に続いて Node.js でも使えるようになった
- `Temporal` は `Instant` / `ZonedDateTime` / `PlainDate` / `PlainTime` / `PlainDateTime` / `Duration` / `Now` などを役割ごとに分けたクラス群で、`Date` の設計上の問題を解消する
...
`setUTCMonth` は元のインスタンス `d` を書き換えます。さらに `1/31` の1ヶ月後は2月に該当日がないので `3/3` として扱われ、月末を1ヶ月進めたいだけの操作で2月が飛びます。`const` で受けても安心できず、関数間で渡し回す際に意図しない書き換えが起きやすい設計。
...
ISO 8601 の `YYYY-MM-DD` は **UTC の0時** として解釈されます。スラッシュ区切りの `YYYY/MM/DD` は **実行環境のローカル0時** として解釈されます。Tokyo (UTC+9) で実行すると、後者は UTC では前日15時となり、文字列が1字違うだけで日付が前後にずれます。形式間でルールが揃っていない上、ローカル時刻として解釈される側は実行環境にも依存。

## Node.js v26 でついに標準化された

Temporal は2017年に TC39 にプロポーザルが上がり、Stage 4（標準入り）に到達しました。各環境の対応状況は次の通りです。
...
Node.js v26 で `Temporal` が見えていることを最小コードで確認します。
...
`Temporal` は `Math` と同じ名前空間オブジェクトで、`new Temporal()` のような直接インスタンス化はできません。すべての操作は `Temporal.Now` や `Temporal.Instant.from()` のように静的メソッド・サブクラス経由で行います。
...
`Date` が黙って `3/2` にしていたところを、`Temporal` は例外で弾きます。同じ思想が時刻にも貫かれていて、`PlainTime.from("25:00")` も例外を投げます。
...
「年」を持たない月日のための型が独立しているので、誕生日の月日のように2/29 を扱えるかどうかを型レベルで区別したい場面でそのまま使えます。
...
同じ `Instant`（絶対的な瞬間）を、タイムゾーンを通して「各タイムゾーンでの日時」に変換しています。逆向きの変換も対称に用意されています。
...
DST の境目をまたぐ場合、「1日 = 24時間」が成立しません。Temporal はこれを素直に扱います。
...
`add({ days: 1 })` の結果はカレンダー上で 12:00 → 12:00 で1日後ですが、実経過時間は **23時間**です。Temporal は `days` と `hours` を意図的に区別していて、`days: 1` は「カレンダー上の1日先」、`hours: 24` は「物理時間で24時間後」と別物として扱います。
...
「1月31日の1ヶ月後」を「2月の存在する最終日」として解釈します。
...
| 同じ Date で UTC とローカルで違う時刻が返る | `Instant`（UTC）と `ZonedDateTime` / `PlainDateTime`（壁時計）で型から分離 |
